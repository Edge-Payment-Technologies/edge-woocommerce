...
class WC_Gateway_Edge extends WC_Payment_Gateway
{
	...
	public function process_payment($order_id)
	{
		...
		try {
			...
			// Bind the demand to the order before confirming it. Edge can deliver
			// the webhook before this request finishes, and the handler needs this
			// metadata to find the order.
			...
			$confirmed_demand = Edge\Client::confirm('payment_demands', $payment_demand_id);
		} catch (Exception $e) {
		...
		// Read the state returned by confirm without polling. Pending payments are
		// completed by the webhook when Edge finishes processing them.
		$processor_state = isset($confirmed_demand->data->attributes->processor_state)
			? (string) $confirmed_demand->data->attributes->processor_state
			: '';

		// A webhook may have changed the order while confirm was in flight.
		$order = wc_get_order($order_id);

		if ('failed' === $processor_state || $order->has_status('failed')) {
		...
		if ('succeeded' === $processor_state && !$order->is_paid()) {
			$order->payment_complete($payment_demand_id);
